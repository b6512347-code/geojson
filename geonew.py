...
if st.session_state.get("show_results", False):
    ...
    with st.spinner(f"📡 กำลังประมวลผลด้วย {algorithm_choice}..."):
        ...
        c5.metric("⛽ เชื้อเพลิงรวม",   f"{activity_A:.2f} ลิตร")
        c6.metric("🌿 คาร์บอนรวม (CO₂e)", f"{carbon_E:.2f} kg")
        ...
        # =====================================================================
        # ตารางปฏิบัติงานรายคัน — พร้อม Carbon รายคันและรายเที่ยว
        # =====================================================================
        st.markdown("<br>", unsafe_allow_html=True)
        ...
        for vehicle_name, trips in fleet_schedule.items():
            ...
            # คำนวณ Carbon ระดับรถ (IPCC Tier 1 / TGO)
            # E = AD × EF × GWP  โดย  AD = ระยะทาง / อัตราสิ้นเปลือง
            vehicle_fuel_used    = total_d / fuel_economy if fuel_economy > 0 else 0.0
            vehicle_carbon_kgco2 = vehicle_fuel_used * ef_value * gwp_value

            with st.expander(
                f"{vehicle_name} — {len(trips)} เที่ยว | "
                f"{total_d:.2f} กม. | {total_v:.2f} ลบ.ม. | "
                f"🌿 {vehicle_carbon_kgco2:.2f} kgCO₂e",
                expanded=True
            ):
                if not trips:
                    st.write("✅ รถคันนี้ไม่ได้ออกปฏิบัติงาน (Standby)")
                else:
                    # Metric summary ระดับรถ
                    col_a, col_b, col_c, col_d = st.columns(4)
                    col_a.metric("📍 ระยะทางรวม",      f"{total_d:.2f} กม.")
                    col_b.metric("🗑️ ปริมาตรรวม",      f"{total_v:.2f} ลบ.ม.")
                    col_c.metric("⛽ เชื้อเพลิงใช้",   f"{vehicle_fuel_used:.2f} ลิตร")
                    col_d.metric("🌿 Carbon (CO₂e)",   f"{vehicle_carbon_kgco2:.2f} kg")

                    st.markdown("---")

                    # รายละเอียดแต่ละเที่ยว พร้อม Carbon รายเที่ยว
                    for t in trips:
                        trip_fuel   = t["dist"] / fuel_economy if fuel_economy > 0 else 0.0
                        trip_carbon = trip_fuel * ef_value * gwp_value

                        st.info(
                            f"**เที่ยวที่ {t['trip_sequence']}**\n\n"
                            f"📍 {nodes[0]} ➡️ {' ➡️ '.join(t['route'])} ➡️ {nodes[0]}\n\n"
                            f"🗑️ ปริมาตร: **{t['vol']:.2f} ลบ.ม.**　|　"
                            f"📏 ระยะทาง: **{t['dist']:.2f} กม.**　|　"
                            f"⛽ เชื้อเพลิง: **{trip_fuel:.2f} ลิตร**　|　"
                            f"🌿 Carbon: **{trip_carbon:.2f} kgCO₂e**"
                        )
