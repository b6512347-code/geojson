...
# =====================================================================
# 6. Interactive Map & GeoJSON Export
# =====================================================================
def generate_geojson(routes, locations, nodes, routing_mode, G_osm=None, algo_name="Algorithm"):
    features = []
        ...
        if routing_mode == "Local Map (ออฟไลน์ผ่านไฟล์ .osm)" and G_osm is not None:
            for k in range(len(full_route) - 1):
                ...
                try:
                    ...
                    for nid in path:
                        road_lonlat.append([G_osm.nodes[nid]['x'], G_osm.nodes[nid]['y']])
                except nx.NetworkXNoPath:
                    road_lonlat.extend([[lon1, lat1], [lon2, lat2]])
        else:
            coord_str = ";".join(f"{coords_lonlat[n][0]},{coords_lonlat[n][1]}" for n in full_route)
            url = (f"http://router.project-osrm.org/route/v1/driving/{coord_str}"
                   f"?overview=full&geometries=geojson")
            try:
                data = requests.get(url, timeout=10).json()
                road_lonlat = data["routes"][0]["geometry"]["coordinates"]
            except Exception:
                road_lonlat = [[coords_lonlat[n][0], coords_lonlat[n][1]] for n in full_route]

        feature = {
            "type": "Feature",
            "properties": {
                "Trip": trip_idx + 1,
                "Vehicle": f"รถคันที่ {trip_idx + 1}",
                "Algorithm": algo_name,
                "Total_Stops": len(route_seq)
            },
            "geometry": {
                "type": "LineString",
                "coordinates": road_lonlat
            }
        }
...
st.markdown("ระบบจัดเส้นทางอัจฉริยะ พร้อมฟังก์ชัน **Load Balancing** และส่งออกไฟล์ GeoJSON สู่ QGIS")
...
if st.session_state.get("show_results", False):
    ...
    with st.spinner(f"📡 กำลังประมวลผลด้วย {algorithm_choice}..."):
        ...
        # --- ปุ่ม Export GeoJSON ---
        geojson_data = generate_geojson(routes, osrm_fmt, nodes, routing_mode, G_osm, algorithm_choice)
        st.download_button(
            label="💾 ดาวน์โหลดเส้นทาง (GeoJSON) นำไปใช้ใน QGIS",
            data=geojson_data,
            file_name=f"routes_{algorithm_choice.split()[0]}.geojson",
