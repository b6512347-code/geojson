...
def routedist_by_name(route_names, depot_name, df_dist):  
    full = [depot_name] + route_names + [depot_name]  
    return sum(df_dist.loc[full[k], full[k+1]] for k in range(len(full) - 1))  
  
  
def twoopt(route_names, depot_name, df_dist, max_iter=500):  
    best = route_names[:]  
    for _ in range(max_iter):  
        ...
        for i in range(len(best) - 1):  
            for j in range(i + 2, len(best)):  
                ...
                if (routedist_by_name(candidate, depot_name, df_dist)  
                        < routedist_by_name(best, depot_name, df_dist)):  
...
def run_sweep_algorithm(locations, demands, nodes, max_capacity, df_dist):  
    ...
    routes = [twoopt(r, depot_name, df_dist) for r in routes]  
    return routes, route_vols  
...
# 6. Export to Excel  
# =====================================================================  
def create_excel_export(df_dist, routes, route_vols, route_distances, 
                       nodes, demands, algorithm_choice, vehicle_workloads,
                       locations, fuel_economy, ef_value, gwp_value, max_capacity):
    """สร้าง Excel ที่มี Distance Matrix และข้อมูลเส้นทาง"""
    
    wb = Workbook()
    
    # ====== Sheet 1: Distance Matrix ======
    ws1 = wb.active
    ws1.title = "Distance Matrix (OSRM)"
    
    # Header
    ws1['A1'] = f"Distance Matrix จาก {algorithm_choice}"
    ...
    # เขียน Distance Matrix
    for col_idx, node in enumerate(nodes, 1):
        ws1.cell(row=2, column=col_idx, value=node)
        ws1.cell(row=2, column=col_idx).font = Font(bold=True, color="FFFFFF")
        ws1.cell(row=2, column=col_idx).fill = PatternFill(start_color="4472C4", 
                                                           end_color="4472C4", 
                                                           fill_type="solid")
    
    for row_idx, node in enumerate(nodes, 3):
        ws1.cell(row=row_idx, column=1, value=node)
        ws1.cell(row=row_idx, column=1).font = Font(bold=True)
        ws1.cell(row=row_idx, column=1).fill = PatternFill(start_color="D9E1F2", 
                                                           end_color="D9E1F2", 
                                                           fill_type="solid")
    
    # ใส่ข้อมูลระยะทาง + ColorScale
    max_dist = df_dist.max().max()
    for i, node_i in enumerate(nodes):
        for j, node_j in enumerate(nodes):
            value = df_dist.loc[node_i, node_j]
            cell = ws1.cell(row=i+3, column=j+1, value=float(value))
            cell.number_format = '0.00'
            cell.alignment = Alignment(horizontal='center')
            
            # Gradient สี
            if i != j and max_dist > 0:
                intensity = value / max_dist
                if intensity < 0.33:
    ...
    ws2['A1'] = f"สรุปเส้นทางทั้งหมด ({algorithm_choice})"
    ...
    headers = ["Trip#", "Route Sequence", "Volume (ลบ.ม.)", "Distance (กม.)", 
               "Avg Load %", "Fuel (ลิตร)", "CO₂e (kg)"]
    ...
    for trip_idx, (route, vol, dist) in enumerate(zip(routes, route_vols, route_distances), 1):
        route_str = f"{nodes[0]} → {' → '.join(route)} → {nodes[0]}"
        fuel = dist / fuel_economy
        carbon = fuel * ef_value * gwp_value
        load_pct = (vol / max_capacity * 100) if max_capacity > 0 else 0
        
        row = trip_idx + 3
        ws2.cell(row=row, column=1, value=trip_idx)
        ws2.cell(row=row, column=2, value=route_str)
        ws2.cell(row=row, column=2).alignment = Alignment(wrap_text=True)
        ws2.cell(row=row, column=3, value=vol).number_format = '0.00'
        ws2.cell(row=row, column=4, value=dist).number_format = '0.00'
        ws2.cell(row=row, column=5, value=load_pct).number_format = '0.0"%"'
        ws2.cell(row=row, column=6, value=fuel).number_format = '0.00'
        ws2.cell(row=row, column=7, value=carbon).number_format = '0.00'
    
    # สรุปทั้งหมด
    total_row = len(routes) + 4
    ws2.cell(row=total_row, column=1, value="รวม")
    ws2.cell(row=total_row, column=1).font = Font(bold=True)
    ws2.cell(row=total_row, column=3, value=sum(route_vols)).number_format = '0.00'
    ws2.cell(row=total_row, column=4, value=sum(route_distances)).number_format = '0.00'
    ws2.cell(row=total_row, column=6, value=sum(route_distances)/fuel_economy).number_format = '0.00'
    ws2.cell(row=total_row, column=7, value=sum(route_distances)/fuel_economy*ef_value*gwp_value).number_format = '0.00'
    
    for col in range(1, 8):
        ws2.cell(row=total_row, column=col).font = Font(bold=True)
        ws2.cell(row=total_row, column=col).fill = PatternFill(start_color="FFF2CC", 
                                                               end_color="FFF2CC", 
                                                               fill_type="solid")
    ...
    # ====== Sheet 3: Vehicle Assignment ======
    ws3 = wb.create_sheet("Vehicle Assignment")
    
    ws3['A1'] = "การมอบหมายงานให้รถแต่ละคัน"
    ...
    headers = ["Vehicle", "Trips", "Total Workload (กม.)", "Utilization %", "Status"]
    for col_idx, header in enumerate(headers, 1):
    ...
    row_num = 3
    total_workload = sum(vehicle_workloads.values())
    
    for vehicle_name, workload in vehicle_workloads.items():
        util_pct = (workload / total_workload * 100) if total_workload > 0 else 0
        status = "Active" if workload > 0 else "Standby"
        
        ws3.cell(row=row_num, column=1, value=vehicle_name)
        ws3.cell(row=row_num, column=2, value="")
        ws3.cell(row=row_num, column=3, value=workload).number_format = '0.00'
        ws3.cell(row=row_num, column=4, value=util_pct).number_format = '0.0"%"'
        ws3.cell(row=row_num, column=5, value=status)
        
        for col in range(1, 6):
            ws3.cell(row=row_num, column=col).font = Font(bold=True)
            ws3.cell(row=row_num, column=col).fill = PatternFill(start_color="E7E6E6", 
                                                                 end_color="E7E6E6", 
                                                                 fill_type="solid")
        row_num += 1
    ...
    # ====== Sheet 4: Demand Details ======
    ws4 = wb.create_sheet("Location Details")
    
    ws4['A1'] = "รายละเอียดจุดเก็บขยะ"
    ...
    headers = ["Location Name", "Latitude", "Longitude", "Demand (ลบ.ม.)", "% of Total"]
    for col_idx, header in enumerate(headers, 1):
    ...
    for row_idx, (node, location_data) in enumerate(zip(nodes, locations), 4):
        demand = demands[node]
        pct = (demand / total_demand * 100) if total_demand > 0 else 0
        
        ws4.cell(row=row_idx, column=1, value=node)
        ws4.cell(row=row_idx, column=2, value=location_data[1]).number_format = '0.000000'
        ws4.cell(row=row_idx, column=3, value=location_data[2]).number_format = '0.000000'
        ws4.cell(row=row_idx, column=4, value=demand).number_format = '0.00'
        ws4.cell(row=row_idx, column=5, value=pct).number_format = '0.0"%"'
...
with st.sidebar:  
    ...
    # เก็บค่าไว้ใน session state
    st.session_state['fuel_economy'] = fuel_economy
    st.session_state['ef_value'] = ef_value
    st.session_state['gwp_value'] = gwp_value
    st.session_state['max_capacity'] = max_capacity
  
# ---- MAIN AREA ----  
st.subheader("📝 1. ข้อมูลพิกัดจุดทิ่งขยะ")  
...
if st.session_state.get("show_results", False):  
    ...
    with st.spinner(f"📡 กำลังประมวลผลด้วย {algorithm_choice}..."):  
        ...
        # =====================================================================  
        st.markdown("<br>", unsafe_allow_html=True)
        st.subheader("💾 5. ดาวน์โหลดผลลัพธ์")
        
        try:
            with st.spinner("📦 กำลังเตรียมไฟล์ Excel..."):
                wb = create_excel_export(
                    df_dist, routes, route_vols, route_distances,
                    nodes, demands, algorithm_choice, vehicle_workloads,
                    osrm_fmt, fuel_economy, ef_value, gwp_value, max_capacity
                )
                ...
                with col_dl1:
                    st.download_button(
                        label="📥 ดาวน์โหลด Excel (รวมทุก Sheet)",
                        data=excel_buffer,
                        file_name=f"Routing_Results_{algorithm_choice.replace(' ', '_')}.xlsx",
                        mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                        use_container_width=True
                    )
                
                with col_dl2:
                    # CSV สำหรับ Distance Matrix
                    csv_buffer = df_dist.to_csv(index=True)
                    st.download_button(
                        label="📥 ดาวน์โหลด CSV (Distance Matrix)",
                        data=csv_buffer,
                        file_name=f"Distance_Matrix_{algorithm_choice.replace(' ', '_')}.csv",
                        mime="text/csv",
                        use_container_width=True
                    )
        except Exception as e:
        ...
        # ตารางตัวอย่าง Distance Matrix
        st.markdown("**📊 ตัวอย่าง Distance Matrix (OSRM API) — แสดง 10x10 แรก:**")
        preview_size = min(10, len(df_dist))
        st.dataframe(
            df_dist.iloc[:preview_size, :preview_size],
            use_container_width=True
        )
        
        st.success("✅ ดาวน์โหลด Excel เพื่อดูผลลัพธ์ฉบับเต็มทั้ง Distance Matrix และเส้นทาง!")
